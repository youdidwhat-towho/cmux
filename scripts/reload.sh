...
if [[ -d "$PWD/ghostty" ]]; then
  ...
  elif [[ -x "$GHOSTTY_HELPER_DEST" ]]; then
    echo "Using Ghostty CLI helper from Xcode app build"
  else
    mkdir -p "$BIN_DIR"
...
if [[ "$LAUNCH" -eq 1 ]]; then
  ...
  if [[ -n "${TAG_SLUG:-}" && -n "${CMUX_SOCKET:-}" ]]; then
    ...
    LAUNCH_CMD=("${OPEN_CLEAN_ENV[@]}" "${TAG_LAUNCH_ENV[@]}" CMUX_SOCKET_PATH="$CMUX_SOCKET" CMUXD_UNIX_PATH="$CMUXD_SOCKET" open -g "$APP_PATH")
    LAUNCH_RETRY_CMD=("${OPEN_CLEAN_ENV[@]}" "${TAG_LAUNCH_ENV[@]}" CMUX_SOCKET_PATH="$CMUX_SOCKET" CMUXD_UNIX_PATH="$CMUXD_SOCKET" open -n -g "$APP_PATH")
