...
env.update(
    {
        "CMUX_THEME_PICKER_CONFIG": config_path,
...
try:
    while time.monotonic() < deadline:
        ...
        if not sent_enter and b"Enter apply" in output:
    ...
    if status is None:
        ...
        if status is None:
            ...
            _, status = os.waitpid(pid, 0)
        sys.stderr.write(
            "FAIL: theme picker did not exit after Enter in cmux mode.\n"
        )
    ...
    if exit_code != 0:
        sys.stderr.write(f"FAIL: theme picker exited with status {exit_code}.\n")
        sys.stderr.write(output[-2000:].decode("utf-8", errors="replace"))
        sys.exit(1)
finally:
    os.close(master_fd)
...
if [ ! -f "$CONFIG_PATH" ]; then
  echo "FAIL: Enter did not write the cmux theme override file" >&2
  exit 1
fi
